...
@allure.feature('Edit data')
@allure.story('Edit an object with PUT')
...
@allure.feature('Edit data')
@allure.story('Edit an object with PATCH')
...
def test_delete_an_obj(setup_and_teardown):
    with allure.step('Deleting an object'):
        response = requests.delete(f'http://objapi.course.qa-practice.com/object/{setup_and_teardown}')
        assert response.status_code == 200, 'Status code is not 200'

    with allure.step('Checking that object has been deleted'):
