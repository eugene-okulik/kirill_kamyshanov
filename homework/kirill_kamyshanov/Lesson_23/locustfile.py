...
class ManipulationsObjects(HttpUser):
    ...
    def create_an_obj(self):
        request = self.client.post('/', json={"name": "stranger","data": {"feature": "value"}})


    def put_edit_obj(self):
        request = self.client.put(f'/{1}', json={"name": "stranger","data": {"feature": "value"}})

    def patch_edit_obj(self):
        request = self.client.patch(f'/{1}', json={"name": "stranger","data": {"feature": "value"}})


    def delete_an_obj(self):
        request = self.client.delete(f'/{1}')
...
# # Создание объекта
# def create_obj():
#     body = {"name": "stranger","data": {"feature": "value"}}
#     response = requests.post('http://objapi.course.qa-practice.com/object', json=body)
